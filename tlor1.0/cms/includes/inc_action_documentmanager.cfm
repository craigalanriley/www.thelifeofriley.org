...
<cfset FieldID = "document_id">

<cfset FileExtList="doc,pdf">
<cfset Root = ExpandPath("../uploads/documents/")>
		...
		<cfswitch expression="#Action#">	
			...
			<cfcase value="Create">				
				...
				<cftransaction>
					...
					<cfif NOT ListFind(FileExtList,file.ServerFileExt)>
						...
						<cfset Message = "Sorry - the file you uploaded must be one of the following: #FileExtList#,1">
						
					<cfelse>
											
						<!--- If file type is ok, add data to DB --->														
						<cfquery name="AddRecord" datasource="#Application.DataSource#">
							INSERT INTO cms_documents(date_created,date_modified,created_by_id,modified_by_id,document_name,file_name,work_flow_id)						
							VALUES(#CreateODBCDateTime(now())#,#CreateODBCDateTime(now())#,#Session.UserID#,#Session.UserID#,'#document_name#','#file.ServerFile#',#work_flow_id#)				
						</cfquery>				
						...
						<cfif isDefined("module_id")>
							<cfloop list="#module_id#" index="x">
								<cfquery name="AddLookups" datasource="#Application.DataSource#">
									INSERT INTO cms_doc_mod_lookup(document_id,module_id)						
									VALUES(#ID#,#x#)				
								</cfquery>		   
							</cfloop>						
						</cfif>
			...
			<cfcase value="Edit">
				...
				<cftransaction>
					...
					<cfquery name="DeleteLookups" datasource="#Application.DataSource#">
						DELETE from cms_doc_mod_lookup
						WHERE #FieldID# IN (#ID#)
					</cfquery>					
					<cfif isDefined("module_id")>
						<cfloop list="#module_id#" index="x">
			...
			<cfcase value="Delete">
				...
				<cftransaction>
					...
					<!--- Delete document from DB but leave on server in case it is linked too in content --->
					<cfquery name="DeleteRecord" datasource="#Application.DataSource#">
						DELETE from cms_documents
						WHERE #FieldID# IN (#ID#)
					</cfquery>		
					<!--- Delete Module Lookups --->
					<cfquery name="DeleteLookups" datasource="#Application.DataSource#">
						DELETE from cms_doc_mod_lookup
						WHERE #FieldID# IN (#ID#)
					</cfquery>	
